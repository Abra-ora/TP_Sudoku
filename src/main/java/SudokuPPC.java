...
//        System.out.println("=========>"+readSudokuFile()+" "+n);
////        for (int i = 0; i < 9; i++) {
//            for (int j = 0; j < 9; j++){
//                System.out.println(sudokuCvs.get(1).split(" ")[j]);
//            }
////        }
...
//        }
        ...
        if(readSudokuFile()){
            ...
            System.out.println("ERROR IN SUDOKU DIMENSION");
        }
    ...
    public static boolean readSudokuFile(){
        boolean sukokuIsCorrect = true;
        int countLines=0;
        try {
            FileReader filereader = new FileReader("/home/ibrahim/Documents/M2 GL/IA pour GL (HAI916I)/" +
                    "com.sudoku.algortihm/src/main/java/sudoku.cvs");
...
//        readSudokuFile();
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++){
                String s = sudokuCvs.get(i).split(" ")[j];
                if (Integer.parseInt(s) != 0) {
                    model.arithm(rows[i][j], "=", Integer.parseInt(s)).post();
                }
            }
        }
