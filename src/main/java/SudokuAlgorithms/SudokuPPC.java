...
public class SudokuPPC {
    ...
    public void AddConstraintsForGreaterThanSudoku(ArrayList<String> sudokuCvs){
        for (int i = 0; i < n; i++) {
    ...
    public void buildModel(ArrayList<String> sudokuCvs, boolean isGreaterThanSudoku) {
        model = new Model();
        rows = new IntVar[n][n];
        cols = new IntVar[n][n];
            ...
            for (int i = 0; i < n; i++) {
                for (int j = 0; j < n; j++){
                    ...
                    if (!Objects.equals(s, "0")) {
                        if(sudokuAlphabet.getAlphabet().containsKey(s)){
                            model.arithm(rows[i][j], "=", sudokuAlphabet.getValue(s)).post();
                        }else{
